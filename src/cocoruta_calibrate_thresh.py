...
def get_legend_name(confidence_method):
    if (
        confidence_method == "frequency+cocoruta"
        or confidence_method == "frequency+cocoruta-ranking"
    ):
        return "Frequency"
    elif confidence_method == "baseline" or confidence_method == "baseline-ranking":
        return "Ordinal"
    elif confidence_method == "cocoruta" or confidence_method == "cocoruta-ranking":
        return "Cocoruta confidence"
...
def get_confidence(entry, method, model, tokenizer):
    """
    ...
    elif method == "cocoruta":
        return [float(subclaim["cocoruta-score"]) for subclaim in entry["claims"]]
    elif method == "frequency":
        return get_frequency_scores(
            model, tokenizer, entry["claims"], entry["prompt"], 5
        )
    # This assumes frequency was already added.
    elif method == "frequency+cocoruta":
        return [
            subclaim["cocoruta-score"] + subclaim["frequency-score"]
            for subclaim in entry["claims"]
        ]
    elif method == "optimal":
        return [
...
def add_scores(calibration_data, filename, confidence_methods, model, tokenizer):
    """
    ...
    for confidence_method in confidence_methods:
        if confidence_method + "-score" not in calibration_data[0]["claims"][0]:
            ...
            for entry in tqdm(calibration_data):
                score_list = get_confidence(entry, confidence_method, model, tokenizer)
                for i, output in enumerate(entry["claims"]):
    ...
    if not os.path.exists(f"data/{dataset_prefix}_annotations.jsonl"):
        ...
        for prompt in tqdm(input_dataset):
            ...
            subclaims = get_subclaims(
                model, tokenizer, output, breakdown_prompt=breakdown_prompt
            )
            claim_list = [
                {
                    "subclaim": subclaim["subclaim"],
                    "cocoruta-score": subclaim["cocoruta-score"],
                    "annotation": "N",
                }
